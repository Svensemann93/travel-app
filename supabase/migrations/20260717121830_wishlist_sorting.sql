-- The wishlist had no order at all: the list came back in whatever order Postgres
-- felt like, which is what made it unreadable once it grew. Sorting by name or
-- rating the client can do on its own, but "most recently added" is a fact only the
-- wish row knows, so the read model has to hand it over.
...
as $$
  select
    ...
    (
      select w.created_at from public.place_wishes w
      where w.place_id = p.id and w.user_id = auth.uid()
    ) as wished_on
