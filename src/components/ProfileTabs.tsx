import { useTranslation } from 'react-i18next'

export type ProfileTab = 'about'

const TABS = [
  { key: 'highlights', enabled: false },
  { key: 'pins', enabled: false },
  { key: 'journals', enabled: false },
  { key: 'map', enabled: false },
  { key: 'about', enabled: true },
] as const

function ProfileTabs({ active }: { active: ProfileTab }) {
  const { t } = useTranslation('profile')

  return (
    <div className="flex flex-wrap gap-1 border-b border-slate-200">
      {TABS.map((tab) => {
        const isActive = tab.enabled && tab.key === active
        return (
          <button
            key={tab.key}
            type="button"
            disabled={!tab.enabled}
            className={`-mb-px flex items-center gap-1.5 border-b-2 px-3 py-2.5 text-sm font-medium transition-colors ${
              isActive ? 'border-[#39BBDE] text-slate-900' : 'border-transparent text-slate-400'
            } ${tab.enabled ? 'hover:text-slate-700' : 'cursor-not-allowed'}`}
          >
            {t(`tabs.${tab.key}`)}
            {!tab.enabled && (
              <span className="rounded-full bg-slate-100 px-1.5 py-0.5 text-[10px] font-medium text-slate-400">
                {t('tabs.soon')}
              </span>
            )}
          </button>
        )
      })}
    </div>
  )
}

export default ProfileTabs
