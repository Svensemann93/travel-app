...
- A place is a point on the map; a visit (`place_visits`) is the event of having been there and carries `rating`, `price_level` and `visited_on`. The owner is simply the first visitor of their own place, so own and foreign visits are the same thing — never add a personal field to `places`. You only pin places you have been to, so every own place has exactly one visit; a place without a visit is out of the year review and passport stats by construction. Planning happens with **other people's** public pins, which land in a trip or on the wishlist without a visit. You cannot pin a place you have not been to — an own place without a visit is a later feature.
...
- `trip_places` may reference **other people's** public places — that is what planning is. RLS asks only that the trip is yours; `is_visitable_place` gates the insert, the same predicate that gates a visit. Snapshot columns (`place_name`, `place_latitude`, …) keep a stop renderable when the place is invisible to the owner-only query, which is always for a foreign pin and permanently once a creator turns theirs private. `toTripPlace` rebuilds a `Place` from the snapshot so no trip component knows the difference.
- `place_wishes` is the wishlist: a wish is the mirror image of a visit — a visit records that you were somewhere, a wish that you want to be. It has no fields beyond the pair `(place_id, user_id)`, so it is added or dropped, never updated. Insert is gated by `is_visitable_place`, the same predicate as a visit, so only public places can be wished. `get_public_places` returns `wished_on`, which drives both the map state and the `added` sort in `sortWishlist`.
...
- Route chunks are hashed, so a deploy invalidates the chunk names held by an open tab. Lazy routes are wrapped in `lazyWithReload`, which reloads once (guarded by `sessionStorage`) when a chunk fails to load; the guard clears on success and prevents reload loops on a genuine 404.
- `vercel.json` must serve the document `no-cache` (`max-age=0, must-revalidate`) and hashed `/assets/*` `immutable`. Without the document header a programmatic `location.reload()` may reuse a stale `index.html` — only a manual Ctrl+R would recover. Header changes take effect on the next deploy only.
