...
- A place is a point on the map; a visit (`place_visits`) is the event of having been there and carries `rating`, `price_level` and `visited_on`. The owner is simply the first visitor of their own place, so own and foreign visits are the same thing — never add a personal field to `places`. You only pin places you have been to, so every own place has exactly one visit; a place without a visit is out of the year review and passport stats by construction. Planning happens with **other people's** public pins, which land in a trip without a visit. Own wish-list pins are a later feature.- All user-facing strings go through i18n (de/en). i18next keys are typed — dynamic keys (e.g. `achievements.${id}.title`) require the id to be a literal union, not `string`. Internal thrown errors stay plain English.
